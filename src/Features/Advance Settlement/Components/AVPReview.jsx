...
const getStatusBadgeClass = (status = '') => {
  if (status === SETTLEMENT_STATUS.PENDING_AVP) return 'bg-yellow-100 text-yellow-700'
  if (status === SETTLEMENT_STATUS.REJECTED)    return 'bg-red-100 text-red-700'
  if (status === SETTLEMENT_STATUS.APPROVED)    return 'bg-green-100 text-green-700'
  return 'bg-blue-100 text-blue-700'
}

const STATUS_ORDER = {
  [SETTLEMENT_STATUS.PENDING_AVP]: 1,
}
...
const AVPReview = () => {
  ...
  const shouldShowClarification = (req) => {
    ...
    if (!req.rejectedBy) {
      return req.status === SETTLEMENT_STATUS.PENDING_AVP
    }
    ...
    if (currentRole) {
      ...
      if (currentRole === 'avp-operations' && (normalizedRejectedBy.includes('avp') || normalizedRejectedBy.includes('avp operations'))) return true
    }
  ...
  const loadQueue = useCallback(async () => {
    try {
      await dispatch(fetchAvpQueue({ page: currentPage, limit: ITEMS_PER_PAGE })).unwrap()
    } catch (err) {
      toast.error(`❌ Failed to load AVP approval queue: ${typeof err === 'string' ? err : err?.message || 'Unknown error'}`)
    }
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setApprovingId(id)
      await dispatch(approveAvp({ id })).unwrap()
      toast.success('✅ Settlement approved — forwarded to VP Operations.')
      loadQueue()
    } catch (err) {
  ...
  const handleReject = async () => {
    try {
      ...
      await dispatch(rejectAvp({ id: rejectId, remarks: remarks.trim() })).unwrap()
  ...
  const isActionAllowed = (s) => s.status === SETTLEMENT_STATUS.PENDING_AVP
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-500 rounded-2xl px-6 py-5 mb-6 shadow">
          <h1 className="text-xl sm:text-2xl font-bold text-white">
            ✅ Advance Settlements – AVP Operations Approval
          </h1>
      ...
      <ManagerClarificationModal
        ...
        data={selectedClarificationReq ? {
          rejectionHistory: selectedClarificationReq.rejectionReason ? {
            by: selectedClarificationReq.rejectedBy || 'AVP Operations',
