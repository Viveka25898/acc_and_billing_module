...
const getStatusBadgeClass = (status = '') => {
  if (status === SETTLEMENT_STATUS.PENDING_VP) return 'bg-yellow-100 text-yellow-700'
...
const STATUS_ORDER = {
  [SETTLEMENT_STATUS.PENDING_VP]: 1,
}
...
const VPReview = () => {
  ...
  const shouldShowClarification = (req) => {
    ...
    if (!req.rejectedBy) {
      return req.status === SETTLEMENT_STATUS.PENDING_VP
    }
    ...
    if (currentRole) {
      ...
      if (currentRole === 'vp-operations' && (normalizedRejectedBy.includes('vp') || normalizedRejectedBy.includes('vp operations'))) return true
    }
  ...
  const loadQueue = useCallback(async () => {
    try {
      await dispatch(fetchVpQueue({ page: currentPage, limit: ITEMS_PER_PAGE })).unwrap()
    } catch (err) {
      toast.error(`❌ Failed to load VP approval queue: ${typeof err === 'string' ? err : err?.message || 'Unknown error'}`)
    }
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setApprovingId(id)
      await dispatch(approveVp({ id })).unwrap()
      toast.success('✅ Settlement approved — forwarded to Account Executive.')
      loadQueue()
    } catch (err) {
  ...
  const handleReject = async () => {
    try {
      ...
      await dispatch(rejectVp({ id: rejectId, remarks: remarks.trim() })).unwrap()
  ...
  const isActionAllowed = (s) => s.status === SETTLEMENT_STATUS.PENDING_VP
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-500 rounded-2xl px-6 py-5 mb-6 shadow">
          <h1 className="text-xl sm:text-2xl font-bold text-white">
            ✅ Advance Settlements – VP Operations Approval
          </h1>
      ...
      <ManagerClarificationModal
        ...
        data={selectedClarificationReq ? {
          rejectionHistory: selectedClarificationReq.rejectionReason ? {
            by: selectedClarificationReq.rejectedBy || 'VP Operations',
