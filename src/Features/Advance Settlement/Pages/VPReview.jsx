...
const VPReview = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      // Filter settlements for current VP:
      // 1. Settlements pending VP approval where this VP is the assignee
      // 2. Clarification submissions that need VP review
      const vpRequests = storedSettlements.filter(settlement => {
        // Check if settlement is assigned to current VP
        const isAssignedToVP = settlement.assignedTo === currentUser.username;
        
        // Check if it's a clarification that needs VP review
        const isClarificationForVP = settlement.status.includes('Clarification Submitted') && 
                                   settlement.history.some(h => 
                                     h.action === 'rejected' && 
  ...
  const getNextApprover = () => {
    // VP approves -> goes to Account Executive
      ...
      <div className="overflow-x-auto">
        ...
        {paginatedRequests.length === 0 && (
          <div className="text-center p-8 text-gray-500">
            No settlements found for review.
          </div>
        )}
