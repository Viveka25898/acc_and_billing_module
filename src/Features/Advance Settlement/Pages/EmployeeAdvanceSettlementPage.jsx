...
const EmployeeAdvanceSettlementPage = () => {
  ...
  const updateUserOSBalance = (employeeId, newBalance) => {
    try {
      ...
      console.log(
        `Updated O/S balance for employee ${employeeId}: ₹${newBalance}`
      );
    } catch (error) {
  ...
  const handleSubmitSettlement = async (excelFile, attachments) => {
    try {
      ...
      const invalidRows = excelData.filter(
        (item) =>
          item["Expense Head"] &&
          !validExpenseHeads.includes(item["Expense Head"])
      );
      ...
      if (totalAmount > osBalance) {
        setError(
          `Note: Settlement exceeds O/S balance by ₹${(
            totalAmount - osBalance
          ).toFixed(2)}. This will create employee liability.`
        );
      }
      ...
      const expenseItemsWithGL = excelData.map((item) => ({
        ...item,
        glCode:
          expenseHeadsMaster[item["Expense Head"]]?.glCode || "X2002002001", // Default to Other Expense
      }));
      ...
      reader.onload = (e) => {
        try {
          ...
          const filteredData = jsonData.filter(
            (row) =>
              row["S. No"] && row["Amount (₹)"] && Number(row["Amount (₹)"]) > 0
          );
          ...
          {submitted ? (
            <div className="text-center p-8">
              ...
              <div className="flex justify-center gap-4">
                ...
                <button
                  onClick={() =>
                    navigate("/dashboard/employee/my-settelment-requests")
                  }
            ...
            <>
              ...
              <div className="bg-blue-50 p-4 rounded-md mb-6">
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm text-blue-700">
                  ...
                  <li>
                    <strong>
                      Click any cell in "Expense Head" column to see dropdown
                      arrow ⬇️
                    </strong>
                  </li>
                  <li>Select category from dropdown (no typing needed!)</li>
                  <li>
                    Fill other details and upload with supporting documents
                  </li>
                  <li>
