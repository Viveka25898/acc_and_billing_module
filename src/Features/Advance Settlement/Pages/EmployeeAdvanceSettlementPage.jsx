...
const EmployeeAdvanceSettlementPage = () => {
  ...
  const updateUserOSBalance = (employeeId, newBalance) => {
    try {
      ...
      const updatedUsers = users.map((user) =>
        user.empId === employeeId ? { ...user, osBalance: newBalance } : user
      )
  ...
  const calculateRealOSBalance = (employeeId) => {
    try {
      ...
      const employeeGLCode = `A3001001001-EMP-${String(employeeId).replace('emp', '').padStart(3, '0')}`
        ...
        if (txn.entries && Array.isArray(txn.entries)) {
          txn.entries.forEach((entry) => {
            if (entry.glCode === employeeGLCode) {
              ...
              console.log(
                `📝 Found entry: Dr ${entry.debit}, Cr ${entry.credit} for ${employeeGLCode}`
              )
            }
          })
        }
      ...
      console.log(
        `💰 Final O/S Balance for ${employeeId}: ${balance} (Debits: ${totalDebits}, Credits: ${totalCredits})`
      )
  ...
  useEffect(() => {
    ...
    if (fullUser) {
      ...
      const transactions = JSON.parse(localStorage.getItem('transactions')) || []
      const employeeTransactions = transactions.filter((txn) =>
        txn.entries?.some((entry) => entry.employeeId === fullUser.empId)
      )
      console.log('🔍 Employee transactions found:', employeeTransactions)
    }
  }, [])
  ...
  const exportTemplate = async () => {
    try {
      ...
      instructionSheet.columns = [
        {
          header: 'EXPENSE SETTLEMENT INSTRUCTIONS',
          key: 'instructions',
          width: 80,
        },
      ]
      ...
      const instructions = [
        ...
        ...Object.entries(expenseHeadsMaster).map(
          ([head, details], idx) => `   ${idx + 1}. ${head} → GL: ${details.glCode}`
        ),
        '',
        '💡 TIPS:',
      ...
      toast.success(
        '✅ Template downloaded with exact GL mapping! Open in Excel to see dropdowns.',
        {
          autoClose: 6000,
        }
      )
  ...
  const handleSubmitSettlement = async (excelFile, attachments) => {
    try {
      ...
      if (invalidRows.length > 0) {
        setError(
          `Invalid expense heads found: ${invalidRows
            .map((r) => r['Expense Head'])
            .join(', ')}. Please use only predefined expense heads.`
        )
        return false
      }
      ...
      if (totalAmount > osBalance) {
        setError(
          `Note: Settlement exceeds O/S balance by ₹${(totalAmount - osBalance).toFixed(
            2
          )}. This will create employee liability.`
        )
      }
            ...
            <>
              ...
              <div className="bg-blue-50 p-4 rounded-md mb-6">
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm text-blue-700">
                  <li>Click "Download Excel File" button above</li>
                  <li>Open file in Microsoft Excel (desktop version)</li>
                  <li>
                    <strong>
                      Click any cell in "Expense Head" column to see dropdown arrow ⬇️
                    </strong>
                  </li>
                  ...
                  <li>
                    ...
                    <strong className="text-green-700">₹{osBalance.toFixed(2)}</strong>
                  </li>
                </ol>
              </div>
