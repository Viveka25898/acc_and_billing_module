...
const STATUS_ORDER = {
  'PENDING_VP':                         1,
  'Pending Account Executive Approval': 2,
  'Rejected':                           3,
  'Rejected by VP Operations':          4,
}

const getStatusBadgeClass = (status = '') => {
  if (status === 'PENDING_VP') return 'bg-yellow-100 text-yellow-700'
  if (status.toLowerCase().includes('rejected')) return 'bg-red-100 text-red-700'
  return 'bg-green-100 text-green-700'
}

const formatStatus = (status = '') => {
  if (status === 'PENDING_VP') return 'Pending Approval'
  return status || '—'
}
...
const VPApproval = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        await dispatch(fetchVPApprovalRequests()).unwrap()
      } catch (err) {
  ...
  useEffect(() => { if (errors.fetchVPRequests) toast.error(`❌ ${errors.fetchVPRequests}`) }, [errors.fetchVPRequests])
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setApprovingId(id)
      await dispatch(vpApprove({ id })).unwrap()
      toast.success('✅ Request approved — forwarded to Account Executive')
    } catch (err) {
  ...
  const isActionAllowed = (req) => req.status === 'PENDING_VP'
    ...
    <div className="px-4 py-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-500 rounded-2xl px-6 py-5 mb-6 shadow">
          ...
          <p className="text-green-100 text-sm mt-0.5">Review and approve / reject advance requests for VP Operations</p>
        </div>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-green-50">
                  {paginatedRequests.map((req) => {
                    ...
                    const formattedReason = formatReasons(req.reasons || req.reason, req.customReason)
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                disabled={loading.vpReject || remarks.trim().length < 5}
