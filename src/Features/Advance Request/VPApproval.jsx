...
const VPApproval = () => {
  ...
  const handleReject = () => {
    ...
    setRequests((prev) =>
      prev.map((req) =>
        req.id === rejectId ? { ...req, status: 'Rejected by VP', remarks, clarification: '' } : req
      )
    );
    setRemarks('');
    setRejectId(null);
  ...
  const isActionAllowed = (req) => {
    return (
      ...
      (req.status === 'Rejected by VP' && req.clarification)
    );
  };
    ...
    <div className="min-h-screen px-4 py-10 bg-white rounded shadow-md">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-2xl font-bold text-green-800 mb-6">
          Advance Requests – VP Approval (Approved by Manager)
        </h2>
