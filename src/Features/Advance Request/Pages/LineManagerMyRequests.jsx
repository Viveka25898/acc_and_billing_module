...
const LineManagerMyRequests = () => {
  ...
  console.log("Line Manager Name:", loggedInUserName);

  useEffect(() => {
    // Get all requests from localStorage
    const stored = JSON.parse(localStorage.getItem('advanceRequests')) || [];
    console.log("All stored requests:", stored);

    // For Line Manager, we need to get requests that they submitted when they were employees
    // Or requests that are currently assigned to them for approval
    // But for "My Requests", we want their own submitted requests
    const userRequests = stored
      .filter((r) => r.employeeId === loggedInUserName || r.employeeUsername === loggedInUserName)
      .sort((a, b) => new Date(b.submittedAt) - new Date(a.submittedAt)) // Sort latest first
      .slice(0, 5); // Keep only last 5
    console.log("Line Manager's requests:", userRequests);
  ...
  return (
    <div className="min-h-screen px-4 py-10 bg-white rounded shadow-md">
      <div className="max-w-5xl mx-auto p-6">
        <h2 className="text-2xl font-bold text-green-800 mb-6">My Advance Requests (Line Manager)</h2>
...
export default LineManagerMyRequests;