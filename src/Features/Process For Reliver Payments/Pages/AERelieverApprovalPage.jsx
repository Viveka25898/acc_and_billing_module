  ...
  const handleBankSelected = async (bankData) => {
    ...
    try {
      ...
      if (pendingApprovalData.type === 'single') {
        ...
        console.log(
          `🔄 Processing batch reliever payment for ${pendingApprovalData.requests.length} requests...`
        )
      ...
      const updatedLocalRequests = updatedAllRequests.filter(
        (req) =>
          req.status === 'Pending Account Executive Approval' ||
          req.status === 'Approved' ||
      ...
      if (pendingApprovalData.type === 'single') {
        ...
        toast.success(
          `✅ ${approvedRequestsData.length} reliever payments approved - ${accountingProcessingResult.message}`
        )
      }
