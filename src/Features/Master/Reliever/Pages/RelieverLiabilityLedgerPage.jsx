...
const RelieverLiabilityLedgerPage = () => {
  ...
  const loadLedgerData = () => {
    try {
      ...
      // Get liability account details for header
      const header = RelieverLedgerService.getRelieverLiabilityAccountDetails()

      // Get all ledger entries for L2001002
      const transactions = RelieverLedgerService.getRelieverLiabilityLedgerEntries()

      // Get summary data
      const summary = RelieverLedgerService.getLiabilitySummaryData(transactions)
      ...
      console.error('Error loading reliever liability ledger data:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const filteredTransactions = useMemo(() => {
    ...
    // Apply all filters
    filtered = RelieverLedgerService.filterByDateRange(filtered, filters.fromDate, filters.toDate)
  ...
  const filteredSummary = useMemo(() => {
    return RelieverLedgerService.getLiabilitySummaryData(filteredTransactions)
    ...
    return (
      <div className="min-h-screen p-4 md:p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading reliever liability ledger data...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen p-4 md:p-6">
      <div className="max-w-5xl mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <Summary summary={filteredSummary} balanceType="CR" />
        </div>
      </div>
    </div>
  )
