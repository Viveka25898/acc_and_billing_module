...
const CGSTInputLedgerPage = () => {
  ...
  const loadLedger = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await GSTLedgerService.getLedgerFor("A3007001001", "CGST Input");
      setLedger(data);
    } catch (err) {
      setError(err.message || "Failed to fetch CGST Input Ledger. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-gray-500 font-semibold">Loading CGST Input Ledger...</p>
          </div>
        )}
