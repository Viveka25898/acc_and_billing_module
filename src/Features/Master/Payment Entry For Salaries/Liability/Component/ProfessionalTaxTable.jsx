...
const ProfessionalTaxTable = ({ summaryData, ledgerData }) => {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="p-5 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-lg font-semibold text-red-600">
          Professional Tax Payable Ledger Entries
        </h2>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {ledgerData.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm font-medium text-red-600">{row.voucherNo}</td>
                ...
                <td className="px-4 py-3 text-sm text-gray-900">{formatDate(row.dueDate)}</td>
                <td className="px-4 py-3 text-sm text-gray-900">{formatDate(row.paymentDate)}</td>
      ...
      <div className="p-4 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="text-sm text-gray-600">
          Showing 1 to {ledgerData.length} of 124 entries | Current Liability: ₹24,800.00
        </div>
        <div className="flex gap-2">
