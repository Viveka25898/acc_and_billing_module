...
const EmployeeESICPayableLedgerPage = () => {
  ...
  useEffect(() => {
    const loadLedgerData = async () => {
      try {
        setLoading(true)
        setError(null)
        console.log('🔄 Loading EMPLOYEE ESIC PAYABLE ledger data...')

        // Get ledger details
        const details = SalaryLedgerService.getLedgerDetails('L2002007')
        if (!details) {
        ...
        // Get transactions
        const transactions = SalaryLedgerService.getLedgerTransactions('L2002007')
        setAllTransactions(transactions || [])

        console.log('✅ Loaded EMPLOYEE ESIC PAYABLE ledger:', {
          details,
        ...
        console.error('❌ Error loading EMPLOYEE ESIC PAYABLE ledger:', error)
  ...
  const calculateSummary = useMemo(() => {
    try {
      const summary = SalaryLedgerService.getLedgerSummary(filteredTransactions, 'L2002007')
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-white p-4 md:p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading employee ESIC payable ledger...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h3 className="text-xl font-semibold text-gray-800">
              Employee ESIC Payable Transactions ({filteredTransactions.length} records)
            </h3>
            <p className="text-sm text-gray-600 mt-1">
              Liability account showing employee ESIC contributions owed
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={() =>
                SalaryLedgerService.exportLedgerToExcel('L2002007', filteredTransactions)
              }
        ...
        {filteredTransactions.length > 0 ? (
          ...
          <div className="text-center py-12 bg-white rounded-xl shadow-lg">
            ...
            <p className="text-gray-500 mb-4">
              No employee ESIC payable transactions match your current filters.
            </p>
            <button
              onClick={() =>
                setFilters({
