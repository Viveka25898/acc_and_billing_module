...
const OtherDeductionsLedgerPage = () => {
  ...
  useEffect(() => {
    try {
      console.log('🔄 Loading Other Deductions Expense ledger data...')
      const details = SalaryLedgerService.getLedgerDetails('X2001001008')
      setLedgerDetails(details)
      const transactions = SalaryLedgerService.getLedgerTransactions('X2001001008')
      setAllTransactions(transactions)
      console.log('✅ Loaded Other Deductions Expense ledger:', {
        details,
        transactionCount: transactions.length,
      })
      setLoading(false)
    } catch (error) {
      console.error('❌ Error loading Other Deductions Expense ledger:', error)
      setLoading(false)
    }
  }, [])
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Other Deductions Expense Ledger...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        />
        <OtherDeductionsTable ledgerData={filteredTransactions} />
        <OtherDeductionsFooter summaryData={summaryData} />
      </div>
    </div>
  )
}
