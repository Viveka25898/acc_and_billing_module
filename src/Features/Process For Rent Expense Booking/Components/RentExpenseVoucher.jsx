...
const RentExpenseVoucher = ({ data = {}, onClose }) => {
  ...
  const header = data.header || {
    ...
    voucherNo:
      data.voucherNo ||
      `EXP-RENT-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`,
    financialYear:
      ...
      <div className="w-full max-w-5xl bg-white rounded-lg shadow-xl overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-green-600 text-white p-3 sm:p-4 flex justify-between items-center">
          <h2 className="text-lg sm:text-xl font-semibold">
            Expense Voucher - Monthly Rent Payment
          </h2>
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="mb-4 sm:mb-6 p-3 rounded-lg text-center bg-green-50 text-green-700">
            <p className="text-lg font-semibold">Rent for {formatMonth(rentDetails.month)}</p>
            <p className="text-sm mt-1">Total Amount: {formatAmount(totalRentAmount)}</p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border border-gray-200 text-xs sm:text-sm">
                <thead className="bg-green-50">
                  <tr>
                    <th className="px-2 sm:px-3 py-2 text-left font-medium text-gray-700 border">
                      Description
                    </th>
                    <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-700 border">
                      Period
                    </th>
                    <th className="px-2 sm:px-3 py-2 text-center font-medium text-gray-700 border">
                      GST Type
                    </th>
                    <th className="px-2 sm:px-3 py-2 text-right font-medium text-gray-700 border">
                      Amount (₹)
                    </th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr className="bg-green-50 font-bold">
                    <td colSpan={3} className="px-2 sm:px-3 py-2 border text-right">
                      Total Rent Amount:
                    </td>
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {siteDetails.siteName}, {siteDetails.location}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Base Rent:</span>
