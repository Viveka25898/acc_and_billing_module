...
export default function PurchaseVoucherModal({ onClose, invoice }) {
  ...
  const purchaseVoucherNo = invoice.purchaseVoucherNo || accountingResult?.purchaseVoucherNo || `PV-2025-${invoice.id?.toString().padStart(3, '0')}`;
  
  // Use actual transaction data if available, otherwise calculate
  const taxableAmount = breakdown?.taxable || Math.round(invoice.totalAmount / (1 + (invoice.gstRate || 18)/100));
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center px-2">
      <div className="bg-white w-full max-w-2xl p-6 rounded-xl shadow-xl relative">
        ...
        <div className="mb-4">
          <p><strong>Voucher No:</strong> {purchaseVoucherNo}</p>
          <p><strong>Date:</strong> {new Date().toLocaleDateString('en-IN')}</p>
          <p><strong>Vendor:</strong> {invoice.vendorName}</p>
          <p><strong>Invoice Ref:</strong> {invoice.invoiceNumber}</p>
          {invoice.vendorGLCode && <p><strong>Vendor GL Code:</strong> {invoice.vendorGLCode}</p>}
        </div>
            ...
            <tbody>
              {entries.map((entry, index) => (
                <tr key={index} className={entry.credit > 0 ? "bg-gray-50 font-medium" : ""}>
                  ...
                  <td className="p-2 text-right border">{entry.debit > 0 ? entry.debit.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}</td>
                  <td className="p-2 text-right border">{entry.credit > 0 ? entry.credit.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}</td>
                  <td className="p-2 border">{entry.glCode}</td>
                </tr>
              ))}
              <tr className="bg-green-50 font-bold border-t-2">
                <td className="p-2 border">Total</td>
                <td className="p-2 text-right border">{totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                <td className="p-2 text-right border">{totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
        ...
        <div className="bg-blue-50 p-3 rounded mb-4">
          <p className="text-sm">
            The invoice has been booked as a Prepaid Asset. Base amount of <strong>₹{taxableAmount.toLocaleString()}</strong> will be amortized over <strong>{invoice.prepaidPeriod || 12} months</strong> starting from <strong>{invoice.prepaidStartMonth || 'current month'}</strong>.
          </p>
          <p className="text-xs text-gray-600 mt-2">
            Monthly amortization: ₹{((invoice.monthlyAmortization || taxableAmount / (invoice.prepaidPeriod || 12))).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </p>
        </div>
