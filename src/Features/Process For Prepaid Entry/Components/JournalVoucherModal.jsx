...
export default function JournalVoucherModal({ onClose, invoice }) {
  ...
  const prepaidStartMonth =
    invoice.prepaidStartMonth ||
    prepaidDetails.prepaidStartMonth ||
    new Date().toISOString().slice(0, 7)

  // Calculate taxable amount (base amount before GST)
  const gstPercentage = invoice.gstRate || 18
  const baseAmount =
    accountingResult?.breakdown?.taxable ||
    Math.round(invoice.totalAmount / (1 + gstPercentage / 100))
  const monthlyExpense =
    invoice.monthlyAmortization ||
    prepaidDetails.monthlyAmortization ||
    Math.round(baseAmount / prepaidPeriod)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
      <div className="w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl">
        ...
        <div className="mb-4">
          ...
          <p>
            <strong>Period:</strong> {prepaidPeriod} months starting {prepaidStartMonth}
          </p>
          <p>
            <strong>Narration:</strong> Monthly amortization for Prepaid Uniform Expense Invoice{' '}
            {invoice.invoiceNumber}
          </p>
          <p className="text-xs text-gray-600 mt-2">
            <strong>Note:</strong> This is a preview. Monthly amortization JV will be created via
            button click as per requirement.
          </p>
        </div>
        ...
        <div className="bg-blue-50 p-3 rounded mb-4">
          <p className="text-sm">
            ...
            will be posted monthly. This will continue for <strong>{prepaidPeriod} months</strong>{' '}
            until the prepaid asset is fully amortized.
          </p>
          <p className="text-xs text-gray-600 mt-2">
