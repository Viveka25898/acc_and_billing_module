...
const MISReportCard = ({
  ...
  const handleTBReport = async (reportType) => {
    try {
      console.log('TB Report clicked:', reportType)
      if (onTBReportTypeClick) {
        await onTBReportTypeClick(reportType)
      }
    } catch (err) {
      console.error('MISReportCard: handleTBReport error', err)
      setError('Failed to generate TB report')
    }
  }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          <div className="flex-shrink-0">
            {isTBReport ? (
              <button
                onClick={onTBGenerateReport}
                className={`w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 text-white text-sm font-medium rounded-md hover:from-green-700 hover:to-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200 shadow-sm hover:shadow-md ${
                  isHovered ? 'scale-105' : ''
                }`}
              >
        ...
        {/* TB Report Options (shown when expanded) */}
        {isTBReport && tbReportExpanded && (
          <div className="pt-3 border-t border-gray-200">
            <div className="flex flex-col gap-3">
              <button
                onClick={() => handleTBReport('TB Report')}
              ...
              <button
                onClick={() => handleTBReport('TB Detailed')}
                className="w-full px-4 py-3 bg-gradient-to-r from-purple-600 to-purple-700 text-white text-sm font-medium rounded-md hover:from-purple-700 hover:to-purple-800 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition-all duration-200 shadow-sm hover:shadow-md flex items-center justify-center gap-2"
              >
                <FiFileText className="w-4 h-4" />
                <span>TB Detailed</span>
              </button>

              <button
                onClick={() => handleTBReport('TB Detailed in Date Range')}
                className="w-full px-4 py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white text-sm font-medium rounded-md hover:from-indigo-700 hover:to-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all duration-200 shadow-sm hover:shadow-md flex items-center justify-center gap-2"
              >
                <FiCalendar className="w-4 h-4" />
        ...
        {/* Selected Period Display (for non-TB reports) */}
        {!isTBReport && selectedPeriod && (
          <div className="pt-3 border-t border-gray-200">
        ...
        {/* Placeholder when no period selected (for non-TB reports) */}
        {!isTBReport && !selectedPeriod && (
          <div className="pt-3 border-t border-gray-200">
        ...
        {/* Placeholder for TB report when not expanded */}
        {isTBReport && !tbReportExpanded && (
          <div className="pt-3 border-t border-gray-200">
            <p className="text-xs text-gray-500 text-center italic">
              Click "Generate Report" to view report options
            </p>
          </div>
        )}
      </div>
