...
export default function AEConveyanceApprovalPage() {
  ...
  useEffect(() => {
    try {
      ...
      const aeRequests = allRequests.filter((request) => request.status === 'Pending AE Approval')
  ...
  const filteredClaims = claims.filter((claim) => {
    const clientMatch =
      filter.client === '' || claim.client?.toLowerCase().includes(filter.client.toLowerCase())
    const dateMatch = filter.date === '' || claim.date.split('T')[0] === filter.date
    ...
    try {
      ...
      const voucherData = {
        ...
        employeeDetails: {
          ...
          submissionDate: claimToApprove.submittedAt
            ? new Date(claimToApprove.submittedAt).toISOString().split('T')[0]
            : 'N/A',
      ...
      // ✅ NEW: Save to Processed Conveyance Requests
      const processedRequest = {
        ...
        paymentStatus: 'Pending Payment', // Initial payment status
        paymentHistory: [],
        glEntries: glEntries,
      }

      // Save to processed conveyance requests
      const existingProcessedRequests =
        JSON.parse(localStorage.getItem('processedConveyanceRequests')) || []
      localStorage.setItem(
        'processedConveyanceRequests',
        JSON.stringify([...existingProcessedRequests, processedRequest])
      )
  ...
  const handleReject = (id, reason) => {
    try {
      ...
      console.log(
        'AE Updated requests after rejection:',
        updatedRequests.find((r) => r.id === id)
      )
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto bg-white rounded-md shadow-md">
      ...
      <ConveyanceFilter filter={filter} setFilter={setFilter} showClientFilter={true} />
