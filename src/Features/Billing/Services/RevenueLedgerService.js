        ...
        try {
            ...
            const consolidatedTxns = relevantTxns.map(txn => {
                ...
                return {
                    ...
                    customer: txn.customer || txn.clientName || clientEntry?.glName || '-',
                    invoiceNumber: txn.invoiceNumber || revenueEntries[0].invoiceNumber || '-',
                    irnNumber: txn.irnNumber || '-',
                    debit: totalDebit,
                    credit: totalCredit,
                    costCenter: revenueEntries[0].costCenter || txn.costCenter || 'HEAD OFFICE',
                    site: revenueEntries[0].site || txn.site || '-',
                    state: txn.state || '-',
                    city: txn.city || '-',
                    branch: txn.branch || '-',
