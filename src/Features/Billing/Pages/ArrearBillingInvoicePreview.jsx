...
import { Download, Send, ArrowLeft, CheckCircle, XCircle, Loader2, FileText } from 'lucide-react'
import { useNavigate, useLocation } from 'react-router-dom'
import emailjs from '@emailjs/browser'
import { saveInvoice } from '../utils/invoiceStorage'

const ArrearBillingInvoicePreview = ({
  formData: propFormData,
  rateChanges: propRateChanges,
  manualLineItems: propManualLineItems,
  ...
  // Use props if provided (for modal view), otherwise get from location state
  const formData = propFormData || location.state?.formData
  const rateChanges = propRateChanges || location.state?.rateChanges
  const manualLineItems = propManualLineItems || location.state?.manualLineItems
  const calculations = propCalculations || location.state?.calculations
  ...
  React.useEffect(() => {
    try {
      if (!formData || !rateChanges || !calculations) {
        setError('Invoice data is missing. Redirecting back...')
  ...
  }, [formData, rateChanges, calculations, navigate])
  ...
  const handleSave = async () => {
    try {
      ...
      // Prepare invoice data in the same format as Auto Billing
      const invoiceData = {
        formData: {
          ...formData,
          customer: formData?.client,
          poWoNumber: `ARR-${Date.now()}`,
          branch: formData?.invoiceDescription || 'Arrear Billing - Rate Change Adjustment',
        },
        rateChanges,
        manualLineItems,
        calculations,
        createdBy: 'Billing Manager',
        source: 'arrear', // Mark as arrear billing
        invoiceNumber: `ARR-${Date.now()}`,
      }

      // Save using the same storage utility as Auto Billing
      const result = saveInvoice(invoiceData, 'proforma')

      if (result.success) {
        setSuccess(`Invoice saved successfully! ID: ${result.invoiceId}`)
        setTimeout(() => {
          setSuccess(null)
  ...
  const handleSendEmail = async () => {
    try {
      ...
      const invoiceDetails = {
        to_name: formData.client,
        to_email: 'client@example.com',
        invoice_number: `ARR-${Date.now()}`,
        invoice_date: formatDate(new Date()),
        invoice_amount: formatCurrency(calculations.grandTotal),
        period_start: formatDate(formData.arrearPeriodStart),
        period_end: formatDate(formData.arrearPeriodEnd),
      }
  ...
  return (
    <div
      className={
        isModalView
          ? ''
          : 'min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-emerald-50 py-6 px-4 sm:px-6 lg:px-8'
      }
    >
      <div className={isModalView ? '' : 'max-w-[1200px] mx-auto'}>
        {/* Header - Hide in modal view */}
        {!isModalView && (
          <>
            <div className="mb-6">
              <button
              ...
              >
                ...
                <span>Back to Arrear Billing Form</span>
              </button>

              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                    Arrear Billing Invoice Preview
                  </h1>
                  <p className="text-sm sm:text-base text-gray-600">
                    Review and finalize the arrear billing invoice
                  </p>
                </div>
              </div>
            </div>
            ...
            {emailStatus && (
              ...
              >
                ...
                <p
                  className={`text-sm ${
                    emailStatus.type === 'success' ? 'text-green-700' : 'text-red-700'
                  }`}
                >
                  {emailStatus.message}
                </p>
        ...
        {/* Invoice */}
        <div
          className="max-w-[210mm] mx-auto bg-white shadow-2xl overflow-hidden print:shadow-none"
          id="invoice-content"
        >
          <div className="border-2 border-gray-300 print:border-black">
            {/* Header - Modern Design */}
            <div className="header-gradient p-6 print:bg-gradient-to-r print:from-emerald-600 print:to-emerald-700">
            ...
            <div className="grid grid-cols-2 gap-0 text-xs bg-gray-50">
              <div className="border-r border-gray-300">
                ...
                {[
                  ['Invoice No.', `PO-MM01-2601783`, true],
                  ['Invoice Date', formatDate(new Date()), false],
              ...
              <div>
                ...
                {[
                  [
                    'Bill For Month(s)',
                    `${formatMonthYear(formData?.arrearPeriodStart)} to ${formatMonthYear(formData?.arrearPeriodEnd)}`,
                    true,
                  ],
                  [
                    'Invoice Period',
                    `${formatDate(formData?.arrearPeriodStart)} to ${formatDate(formData?.arrearPeriodEnd)}`,
                    false,
                  ],
                ].map(([label, value, highlight], i) => (
                  <div
            ...
            {/* Bill To / Ship To - Enhanced Cards */}
            <div className="grid grid-cols-2 gap-0 text-xs border-t-2 border-emerald-600">
              {[
                ...
                <div
                  key={idx}
                  className={`${idx === 0 ? 'border-r border-gray-300' : ''} bg-white`}
                >
                  ...
                  <div className="p-4 space-y-1">
                    <p className="font-bold text-gray-800 text-sm mb-2">
                      {formData?.client || 'ABC Mail'}
                    </p>
                    <p className="text-gray-700">Mumbai Branch</p>
                    <p className="text-gray-600 text-[11px] leading-relaxed">
                      • Ground Floor, ABC Mail - Ground Floor
                    </p>
                    <p className="text-gray-600 text-[11px] leading-relaxed">
                      • First Floor, ABC Mail - First Floor
                    </p>
                ...
                <tbody className="bg-white">
                  {rateChanges?.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="border border-gray-300 p-2 text-center font-medium">
                        {item.employeeCount}.00
                      </td>
                      <td className="border border-gray-300 p-2 text-right font-medium">
                        {formatCurrency(item.newDailyRate * 30)}
                      </td>
                      ...
                      <td className="border border-gray-300 p-2 text-right text-blue-700">
                        {formatCurrency(item.arrearAmount * 0.09)}
                      </td>
                  ...
                  {manualLineItems?.map((item, index) => (
                    <tr
                      key={item.id}
                      className={`${
                        (rateChanges.length + index) % 2 === 0 ? 'bg-gray-50' : 'bg-white'
                      } hover:bg-emerald-50 transition-colors`}
                    >
                      <td className="border border-gray-300 p-2 text-center font-medium">
                        {rateChanges.length + index + 1}
                      </td>
                      <td className="border border-gray-300 p-2 text-gray-800">
                        {item.description}
                      </td>
                      ...
                      <td className="border border-gray-300 p-2 text-right text-blue-700">9.00</td>
                      <td className="border border-gray-300 p-2 text-right text-blue-700">
                        {formatCurrency(item.amount * 0.09)}
                      </td>
                      <td className="border border-gray-300 p-2 text-right text-purple-700">
                        9.00
                      </td>
                  ...
                  {[
                    ...Array(
                      Math.max(0, 7 - (rateChanges?.length || 0) - (manualLineItems?.length || 0))
                    ),
                  ].map((_, i) => (
                    <tr
                      key={`e${i}`}
                      className={`${
                        ((rateChanges?.length || 0) + (manualLineItems?.length || 0) + i) % 2 === 0
                          ? 'bg-gray-50'
                          : 'bg-white'
                      }`}
                    >
                  ...
                  <tr className="font-bold bg-gradient-to-r from-emerald-600 to-emerald-700 text-white">
                    <td colSpan="7" className="border border-emerald-800 p-2 text-right text-sm">
                      TOTAL AMOUNT
                    </td>
                    <td className="border border-emerald-800 p-2 text-right text-sm">
                      {formatCurrency(calculations?.totalArrear + (calculations?.manualTotal || 0))}
                    </td>
                    <td className="border border-emerald-800 p-2"></td>
                    <td className="border border-emerald-800 p-2 text-right text-sm">
                      {formatCurrency(calculations?.gstAmount / 2)}
                    </td>
                    <td className="border border-emerald-800 p-2"></td>
                    <td className="border border-emerald-800 p-2 text-right text-sm">
                      {formatCurrency(calculations?.gstAmount / 2)}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg shadow-sm">
                    ...
                    <span className="font-bold text-gray-800 text-base">
                      {formatCurrency(calculations.subtotal + calculations.totalTax)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg shadow-sm">
                    ...
                    <span className="font-bold text-gray-800 text-base">
                      {formatCurrency(
                        Math.round(calculations.grandTotal) - calculations.grandTotal
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-4 bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-lg shadow-md">
                    ...
                    <span className="font-bold text-white text-xl">
                      {formatCurrency(Math.round(calculations.grandTotal))}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-gray-700 leading-relaxed">
                    Towards bill for Housekeeping Services rendered at{' '}
                    <span className="font-semibold">{formData.customer}</span> Location for the
                    period from{' '}
                    <span className="font-semibold">
                      {formatDate(formData.selectedBillingCycle?.cycleFrom)}
                    </span>{' '}
                    to{' '}
                    <span className="font-semibold">
              ...
              <div className="p-6 border-r border-gray-300 bg-white">
                ...
                <div className="space-y-2 text-[10px] leading-relaxed text-gray-700">
                  ...
                  <div className="flex">
                    ...
                    <p>
                      As per the provision of MSME and registered under MSME act, delay in payment
                      of 45 days can lead to Interest claim at a rate which is 3 times the existing
                      bank rate as notified by RBI along with filing litigation proceedings as per
                      the said act.
                    </p>
                  </div>
                  <div className="flex">
                    ...
                    <p>
                      ...
                      <span className="font-semibold">3 days</span> from the date of submission of
                      the bill, No Corrections will be entertained thereafter.
                    </p>
                  </div>
                  <div className="flex">
...
export default ArrearBillingInvoicePreview
