// HouseKeeping Charges Transaction Table Component
import React from 'react'
import Badge from '../../Components/Badge'
import { FileText, MapPin } from 'lucide-react'

const HKChargesTransactionTable = ({ transactions = [] }) => {
  if (transactions.length === 0) {
    return (
      <div className="p-6 text-center text-gray-500">
        <p>No transactions found.</p>
        <p className="text-sm mt-2">HK billing transactions will appear here.</p>
      </div>
    )
  }
        ...
        <tbody>
          {transactions.map((txn, index) => (
            <tr key={index} className="hover:bg-slate-50 border-b border-slate-100">
              ...
              <td className="px-3 py-4 text-sm font-bold font-mono text-right text-blue-800">
                {txn.balance}
              </td>
...
export default HKChargesTransactionTable
