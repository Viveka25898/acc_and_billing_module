// TDS Receivable Ledger Header Component
import React from 'react'

const TDSReceivableLedgerHeader = ({ ledgerInfo }) => {
  if (!ledgerInfo) return <div>Loading ledger details...</div>

  return (
    <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 text-white p-6 sm:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          <div className="text-2xl sm:text-3xl font-bold mb-2">TDS RECEIVABLE LEDGER (194J)</div>
          <div className="text-sm opacity-90">{ledgerInfo.company}</div>
        </div>
        <div className="mt-4 sm:mt-0">
          <div className="bg-cyan-700 bg-opacity-50 px-4 py-2 rounded-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="flex flex-col">
          ...
          <div className="text-base font-semibold">Section 194J @ 10%</div>
        </div>
      </div>
