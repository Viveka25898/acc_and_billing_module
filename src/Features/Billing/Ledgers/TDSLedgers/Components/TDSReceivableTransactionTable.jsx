// TDS Receivable Transaction Table Component
import React from 'react'
import Badge from '../../Components/Badge'
import { FileText } from 'lucide-react'

const TDSReceivableTransactionTable = ({ transactions = [] }) => {
  if (transactions.length === 0) {
    return (
      <div className="p-6 text-center text-gray-500">
        <p>No transactions found.</p>
        <p className="text-sm mt-2">TDS receivable transactions will appear here.</p>
      </div>
    )
  }
          ...
          {transactions.map((txn, index) => (
            <tr key={index} className="hover:bg-slate-50 border-b border-slate-100">
              ...
              <td className="px-3 py-4 text-sm">
                <span className="text-cyan-600 font-semibold cursor-pointer hover:underline flex items-center gap-1">
                  {txn.voucherNo}
              ...
              <td
                className={`px-3 py-4 text-sm font-semibold font-mono text-right ${txn.debit !== 0 ? 'text-green-600' : 'text-gray-400'}`}
              >
                {txn.debit !== 0
              ...
              <td
                className={`px-3 py-4 text-sm font-semibold font-mono text-right ${txn.credit !== 0 ? 'text-red-600' : 'text-gray-400'}`}
              >
                {txn.credit !== 0
                  ? txn.credit.toLocaleString('en-IN', { minimumFractionDigits: 2 })
                  : '-'}
              </td>
              <td className="px-3 py-4 text-sm font-bold font-mono text-right text-cyan-800">
                {txn.balance}
              </td>
              ...
              <td className="px-3 py-4 text-sm font-mono text-right">{txn.invoiceAmount}</td>
...
export default TDSReceivableTransactionTable
