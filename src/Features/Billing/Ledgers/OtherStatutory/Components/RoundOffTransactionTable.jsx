// Round Off Transaction Table Component
import React from 'react'
import Badge from '../../Components/Badge'

const RoundOffTransactionTable = ({ transactions = [] }) => {
  if (transactions.length === 0) {
    return (
      <div className="p-6 text-center text-gray-500">
        <p>No transactions found.</p>
        <p className="text-sm mt-2">Round off transactions will appear here.</p>
      </div>
    )
  }
          ...
          {transactions.map((txn, index) => (
            <tr key={index} className="hover:bg-slate-50 border-b border-slate-100">
              ...
              <td className="px-3 py-4 text-sm">
                <span className="text-gray-600 font-semibold cursor-pointer hover:underline">
                  {txn.voucherNo}
                </span>
              </td>
              <td className="px-3 py-4 text-sm">
              ...
              <td className="px-3 py-4 text-sm font-bold font-mono text-right text-gray-800">
                {txn.balance}
              </td>
              ...
              <td className="px-3 py-4 text-sm font-mono text-right text-green-600">
                {txn.roundedAmount}
              </td>
              <td className="px-3 py-4 text-sm">
...
export default RoundOffTransactionTable
