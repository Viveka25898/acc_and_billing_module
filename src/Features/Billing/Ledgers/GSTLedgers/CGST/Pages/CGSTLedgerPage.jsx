...
const CGSTLedgerPage = () => {
  ...
  const loadLedgerData = async () => {
    try {
      ...
      const glCode = 'L3001' // CGST PAYABLE
      const data = GSTLedgerService.getGSTLedgerWithTransactions(glCode)

      if (!data || data.error) {
        throw new Error(data?.error || 'Failed to load CGST ledger data')
      }
      ...
      console.error('❌ Error loading CGST ledger:', err)
      setError('Failed to load CGST ledger data. Please try again.')
      setLedgerData(null)
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen w-full bg-gray-50 p-4 sm:p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">{error || 'Unable to load CGST ledger data.'}</p>
        </div>
      </div>
    )
  }
