// CGST Transaction Table Component
import React from 'react'
import Badge from '../../../Components/Badge'
import { FileText, MapPin } from 'lucide-react'

const CGSTTransactionTable = ({ transactions = [] }) => {
  if (transactions.length === 0) {
    return (
      <div className="p-6 text-center text-gray-500">
        <p>No transactions found.</p>
        <p className="text-sm mt-2">CGST transactions will appear here.</p>
      </div>
    )
  }
          ...
          {transactions.map((txn, index) => (
            <tr key={index} className="hover:bg-slate-50 border-b border-slate-100">
              ...
              <td className="px-3 py-4 text-sm">
                <span className="text-red-600 font-semibold cursor-pointer hover:underline flex items-center gap-1">
                  {txn.voucherNo}
              ...
              <td className="px-3 py-4 text-sm font-bold font-mono text-right text-red-800">
                {txn.balance}
              </td>
              ...
              <td className="px-3 py-4 text-sm font-mono text-right">{txn.taxableAmount}</td>
              <td className="px-3 py-4 text-sm font-medium">{txn.invoiceNo}</td>
              <td className="px-3 py-4 text-sm">
...
export default CGSTTransactionTable
