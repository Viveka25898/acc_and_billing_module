// CGST Payable Ledger Header Component
import React from 'react'

const CGSTLedgerHeader = ({ ledgerInfo }) => {
  if (!ledgerInfo) {
    return <div>Loading ledger details...</div>
  }

  return (
    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white p-6 sm:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          <div className="text-2xl sm:text-3xl font-bold mb-2">CGST PAYABLE LEDGER</div>
          <div className="text-sm opacity-90">{ledgerInfo.company}</div>
        </div>
        <div className="mt-4 sm:mt-0">
          <div className="bg-red-700 bg-opacity-50 px-4 py-2 rounded-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="flex flex-col">
          ...
          <div className="text-base font-semibold">CGST @ 9%</div>
        </div>
      </div>
...
export default CGSTLedgerHeader
