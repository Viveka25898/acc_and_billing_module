...
import ManualPurchaseEntryModal from './ManualPurchaseEntryModal'
import { toast } from 'react-toastify'

export default function AEInvoiceApproval({ invoice: invoiceProp }) {
  ...
  const [entryType, setEntryType] = useState(
    initialStatus === 'Auto Booked'
      ? 'Auto Entry'
      : initialStatus === 'Manual Entry Submitted'
        ? 'Manual Entry'
        : ''
  )
  ...
  return (
    <div className="w-full bg-white shadow rounded p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        ...
        {/* <PurchaseEntryStatusBadge status={status} /> */}
      </div>
      ...
      {isActionDone && <p className="text-sm text-gray-700 mt-1 font-medium">{entryType}</p>}

      {showModal && (
        <ManualPurchaseEntryModal
          invoice={invoice}
